    ...
    <div class="bg-white rounded shadow p-6">
      ...
      <div class="mt-4">
        <form action="{{ route('posts.comment', $post->id) }}" method="POST" class="space-y-2">
          ...
          <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Commenter</button>
        </form>

        <ul class="mt-4 space-y-2">
          @foreach ($post->comments as $comment)
            <li class="text-sm text-gray-600 border-l-4 border-green-500 pl-3">
              {{ $comment->content }}
            </li>
          @endforeach
        </ul>
